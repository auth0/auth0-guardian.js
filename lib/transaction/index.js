...
function transaction(data, options) {
  ...
  var isAbleToStart = transaction.isAbleToStartWithState(data);
  if (!isAbleToStart.result) {
    throw isAbleToStart.error;
  }
 ...
 * @private
 *
 * Returns an object `{ result, error }` where `result` is true if
 * the transaction is able to start for the given state and false otherwise
 *
 * `error` holds the validation error (if result = false), null otherwise.
 *
 * @param {array.<string>} data.availableEnrollmentMethods
 * @param {array.<string>} data.availableAuthenticationMethods
 */
transaction.isAbleToStartWithState = function isAbleToStartWithState(data) {
  var availableEnrollmentMethods = data.availableEnrollmentMethods;
  var availableAuthenticationMethods = data.availableAuthenticationMethods;

  if (!availableEnrollmentMethods || availableEnrollmentMethods.length === 0 ||
  !availableAuthenticationMethods || availableAuthenticationMethods.length === 0) {
    return { result: false, error: new errors.NoMethodAvailableError() };
  }
