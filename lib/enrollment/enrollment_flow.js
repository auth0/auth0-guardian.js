...
module.exports = class EnrollmentFlow {
  ...
  forFactor(type) {
    ...
    strategy.onCompletion((enrollmentPayload) => {
      this.hub.emit('enrollment-complete', enrollmentPayload);

      if (enrollmentPayload.transactionComplete) {
        process.nextTick(() => this.hub.emit('login-complete', {
          factor: enrollmentPayload.factor,
          recovery: false,
          accepted: true,
          loginPayload: enrollmentPayload.loginPayload
        }));
      }
    });
